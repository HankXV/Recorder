...
public class TableChecker {
    ...
    public TableCheckResult checkTables(Connection con, boolean checkBeanSuperClass) throws SQLException {
        ...
        for (Entry<String, Class<?>> entry : tables.entrySet()) {
            ...
            if (!sqlTableNames.contains(tableName)) {
                ...
                sb.append(tableName).append("[").append(bean.getSimpleName()).append("]").append("：表缺失！").append(line);
                result.error = true;
                continue;
            }
            ...
            for (String dbColumnName : columnDefine.getColumnInfos().keySet()) {
                ...
                if (!contains) {
                    sb.append(tableName).append("[").append(bean.getSimpleName()).append("]").append("：数据库多出字段：'").append(dbColumnName).append("'！")
                            .append(line);
                    result.error = true;
                }
            }
    ...
    public void registTable(String packageName, Class<?> superClass)
        ...
        List<Class<?>> classes = ReflectionUtil.getClassesByPackage(packageName, superClass);
